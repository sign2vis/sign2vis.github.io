...
class Sign2vis_ts_Model(nn.Module):
    '''
    A transformer-based Seq2Seq model.
    '''
    def __init__(self,
...
class Sign2visModel_jn(nn.Module):
    '''
    A transformer-based Seq2Seq model.
    '''
    def __init__(self,
    ...
    def make_visibility_matrix(self, src):
        '''
        building the visibility matrix here
        '''
        # src = [batch size, src len]
        batch_matrix = []
        for each_src in src:
            v_matrix = np.ones([each_src.shape[0]] * 2, dtype=int)
            n_heads_matrix = [v_matrix] * 8 # TODO: 8 is the number of heads ...
            batch_matrix.append(np.array(n_heads_matrix))
        batch_matrix = np.array(batch_matrix)
    ...
    def forward(self, video_array, video_array_mask, trg):
        ...
        video_array_mask = video_array_mask.unsqueeze(1).unsqueeze(2)
        trg_mask = self.make_trg_mask(trg)
        
        # print(video_array_mask.shape, src_mask.shape)
        x_embed, x_mask = x1_embed, video_array_mask

        # print(x_embed.shape, x_mask.shape, tok_types.shape)

        batch_visibility_matrix = self.make_visibility_matrix(x_embed)
        ...
        x_hidden, enc_attention = self.encoder(x_embed, x_mask, batch_visibility_matrix)
    ...
    def forward_encoder(self, video_array, video_array_mask):

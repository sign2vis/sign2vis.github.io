...
class AllEncoder(nn.Module):
    ...
    def forward(self, src, src_mask, tok_types, batch_matrix):
        ...
        src = self.dropout((src * self.scale) + self.tok_types_embedding(tok_types) + self.pos_embedding(pos))
        
        # print(src.shape, src_mask.shape, batch_matrix.shape)
        # src = [batch size, src len, hid dim]

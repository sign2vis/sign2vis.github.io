...
class Encoder_2(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            x = layer(x, mask)
            all_x.append(x)
        return self.norm(all_x[-2]), self.norm(x)
...
def make_transformer_encoder_2(N_layer=4, d_model=256, d_ff=1024, heads=8, dropout=0.1, ffn_layer='conv_relu_conv',
                             first_kernel_size=9):
    "Helper: Construct a transformer encoder from hyperparameters."
    c = copy.deepcopy
    attn = MultiHeadedAttention(heads, d_model)
    if ffn_layer == 'conv_relu_conv':
        ...
        ff = PositionwiseFeedForward(d_model, d_ff, dropout)
    model = Encoder_2(EncoderLayer(d_model, c(attn), c(ff), dropout), N_layer)
    # Initialize parameters with Glorot / fan_avg.
    for p in model.parameters():
        if p.dim() > 1:
            # nn.init.xavier_uniform(p)

...
def guide_decoder_by_candidates(db_id, table_id, trg_field, input_source, table_columns, db_tables_columns_types, topk_ids, topk_tokens,
    ...
    if current_token_type == 'y_axis':
        ...
        y = best_token
        # print(y, table_columns, (y not in table_columns))
            ...
            if is_in_topk == False:
                for col in table_columns:
                    if col != selected_x and col != selected_y and db_tables_columns_types!=None and db_tables_columns_types[db_id][table_id][
                        ...
                        best_id = trg_field.vocab.stoi[best_token]
                        break
        # print(db_id, table_id, best_token)
...
def translate_just_nl(db_id, table_id, sentence, src_field, trg_field, SRC, model,
                                     db_tables_columns, db_tables_columns_types, device, max_len=150, show_progress = False):
    model.eval()
    ...
    with torch.no_grad():
        enc_src, enc_attention = model.encoder(src_tensor, src_mask, batch_visibility_matrix)
...
def translate_s2v(enc_src, src_mask, db_id, table_id, sentence, trg_field, model,
                                     db_tables_columns, db_tables_columns_types, device, max_len=150, show_progress = False):

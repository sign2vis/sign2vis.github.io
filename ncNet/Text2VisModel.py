...
class Text2VisModel(object):
    def __init__(self, trained_model):
        ...
        self.SRC, self.TRG, self.TOK_TYPES, BATCH_SIZE, train_iterator, valid_iterator, test_iterator, self.my_max_length = build_vocab(
            data_dir='./dataset/my_last_data_final/',
        ...
        self.Text2VisModel = Seq2Seq_wo(enc, dec, self.SRC, SRC_PAD_IDX, TRG_PAD_IDX, self.device).to(self.device)  # define the transformer-based ncNet
        self.Text2VisModel.load_state_dict(torch.load(trained_model, map_location=self.device))
    ...
    def text2vis(self, nl_question, chart_template=None, show_progress=None, visualization_aware_translation=True):
        ...
        if visualization_aware_translation == True:
            ...
            try:
                pred_query, attention, enc_attention = translate_sentence_with_guidance(
                    ...
                    self.SRC, self.Text2VisModel, self.db_tables_columns, self.db_tables_columns_types, self.device, self.my_max_length, show_progress
                )
            except:
                pred_query,  attention, enc_attention = translate_sentence(
                    input_src, self.SRC, self.TRG, self.TOK_TYPES, token_types, self.Text2VisModel, self.device, self.my_max_length
                )                
...
if __name__ == '__main__':
    Text2VisModel = Text2VisModel(
        trained_model='./save_models/last_model_best_wo.pt'
    )
    Text2VisModel.specify_dataset(
        data_type='sqlite3',
        db_url='./dataset/database/car_1/car_1.sqlite',
        table_name='cars_data'
    )
    res = Text2VisModel.text2vis(
        nl_question='Bar chart showing the average weight.',
        chart_template=None
    )
    print(res[1])
